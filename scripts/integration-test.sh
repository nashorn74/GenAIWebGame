...
assert_status "POST /api/npcs (shop) → 201" \
  ...
  -d '{"name":"TestMerchant","npc_type":"shop","map_key":"city1","x":100,"y":100,"dialog":"Buy!","gender":"male","race":"human","job":"merchant"}'
...
# 5-6. 골드 부족 구매 실패 (무기 200G, 잔여 골드 < 200)
...
assert_status "GET /api/npcs/:id → 200" \
  GET "$BASE/api/npcs/$NPC_ID" "200"

NPC_NAME=$(json_field "['name']")
